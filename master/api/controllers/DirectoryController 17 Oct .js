...
*/
        workgroups: function(req, res) {
            ...
            Directory.whoseParentIs({ // Get workgroups
                ...
                async.forEach(dirs, function(dir, cb) {dir.recalculateSize(cb);}, function(err){afterwards(err, dirs);});
            });
        ...
        dispatchAPI: function(req, res) {
            ...
            if (!_.isUndefined(req.param('id'))) {
                switch (req.method) {
                    ...
                    case 'PUT':
                      return sails.policies.can('write')(req, res, function(){DirectoryController.update(req, res);});

                    case 'DELETE':
                      return sails.policies.can('admin')(req, res, function(){DirectoryController['delete'](req, res);});
            ...
            switch (req.method) {
              ...
              case 'POST':
                return sails.policies.can('write')(req, res, function(){DirectoryController.mkdir(req, res);});
...
// accordingly.
      ...
      copy: function (req, res) {
        ...
        Directory.find(req.param('id')).done(function (err, srcDir) {
          ...
          if (req.param('name')) destDirName = req.param('name');
          else destDirName += srcDir.name;

          Directory.create({name: destDirName}).done(function (err, destDir) {
            srcDir.cp(destDir, function (err, dir) {
              if (err) return res.json({error: err});
              else return res.json(destDir);
            });
          });
        });
      },
      ...
      quota: function (req, res) {
        ...
        } else return res.json({error: 'No id provided'});
      },
      ...
      setQuota: function (req, res) {
        ...
        Directory.find(req.param('id')).done(function (err, dir) {
          ...
          if (!dir.isWorkgroup) return res.json({
          ...
          if (dir.size > req.param('quota')) return res.json({
