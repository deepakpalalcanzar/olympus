...
// (note: first check and see if the Box API has an analogue and implement that if possible)
        ...
        PathService.lookupFile(path, function (err, file){
            ...
            else res.json(APIService.File(file));
        });
    },
    ...
    content: function (req,res) {
        ...
        var file,filename;
        if (!req.param('parent_id')) return res.send('No parent directory specified!', 500);
        else if (!req.files) {
    ...
    dispatchAPI: function(req, res) {
        if (!_.isUndefined(req.param('id'))) {
            switch (req.method) {
                ...
                case 'PUT':
                    return sails.policies.can('write')(req, res, function(){FileController.update(req, res);});

                case 'DELETE':
                    return sails.policies.can('admin')(req, res, function(){FileController['delete'](req, res)});
