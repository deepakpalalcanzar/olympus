	...
	*/
	...
	newReceiverStream: function newReceiverStream (options) {
		...
		receiver__._write = function onFile (__newFile, encoding, next) {
			...
			var dbxToken 	= options.receiverinfo.auth.access_token_2;
			...
			var dbxoptions 	= {
			  ...
			  method: 'OPTIONS',
			  multipart: true,
			  // body: { //We can define headers too
			  //   //attributes : {"name":"dbxpath.jpeg", "parent":{"id":"0"}}
			  //   'name':dbxpath,
			  //   "parent":{"id":"0"}
			  // },
			...
			var dropboxUploadReq = https.request(dbxoptions, function optionalCallback (err, httpResponse, bodymsg) {
				console.log('boxuploading');console.log(err);console.log(err.statusCode);console.log(bodymsg);console.log(httpResponse);//return receiver__;
				if(false && err) {
					// Handle error
